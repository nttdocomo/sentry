...
class UserNotificationDetailsSerializer(serializers.Serializer):
    ...
    def serialize(self, data):
        ...
        for key in data:
            ...
            resp[db_key] = six.text_type(int(data[key]))
...
class UserNotificationDetailsEndpoint(UserEndpoint):
    ...
    def put(self, request, user):
        ...
        if serializer.is_valid():
            serialized = serializer.serialize(serializer.object)

            for item in serialized:
                (uo, created) = UserOption.objects.get_or_create(user=user, key=item, project=None)
                uo.update(value=serialized[item])
